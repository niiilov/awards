...
const updateCandidateConviction = async (candidateId: string, hasConviction: boolean) => {
  setLoading(true);
  setError(null);
  ...
  try {
    ...
    // Пробуем разные форматы для false
    const dataFormats = [
    ...
    for (const updateData of dataFormats) {
      try {
        ...
        const response = await api.put(`/candidates-conviction/${candidateId}`, updateData);
        ...
        // Продолжаем только если это ошибка валидации для false
        if (formatError.response?.status === 400 && formatError.response?.data?.error?.includes('required')) {
          continue;
        } else {
          throw formatError;
        }
